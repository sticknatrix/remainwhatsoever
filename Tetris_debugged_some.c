...
int drawscreen(int nodraw)
{
 ...
 while(n<9)
 {
 	if ( hvflip==1)
 	{
		 ...
		 ||ram[7*16+9+1+128]<=4)
 	{
	  (ram[128+(ram[7*16+9+128]*7> 15*7 ? 15*7 : ram[7*16+9+128]*7)+ram[7*16+9+1+128]+(n)%3+((n)/3)*7]==1 && ram[7*16+128+(8-n)]==1)/*||ram[7*16+9+128]>13*/ ? errorfill=1 : 0;//pure?
      //if(errorfill==1)break;
    (screen[(ram[7*16+9+128]*7> 15*7 ? 15*7 : ram[7*16+9+128]*7)+ram[7*16+9+1+128]+(n)%3+((n)/3)*7]=ram[7*16+128+(8-n)]==1? 219 :
	screen[(ram[7*16+9+128]*7> 15*7 ? 15*7 : ram[7*16+9+128]*7)+ram[7*16+9+1+128]+(n)%3+((n)/3)*7]);
    }else posreset=1;
   }
   else
   	if ( hvflip==0)
 	{
 		if ((ram[7*16+128+(n)]==0||ram[7*16+9+1+128]+(n)%3<=6)
		 ||ram[7*16+9+1+128]<=4)
     {
      ...
      //if(errorfill==1)break;
    (screen[(ram[7*16+9+128]*7> 15*7 ? 15*7 : ram[7*16+9+128]*7) +ram[7*16+9+1+128]+n%3+(n/3)*7]=ram[7*16+128+n]==1? 219 : screen[(ram[7*16+9+128]*7> 15*7 ? 15*7 : ram[7*16+9+128]*7) +ram[7*16+9+1+128]+n%3+(n/3)*7]);
    }else posreset=1;
   }
   n++;
 ...
 videoclear();
  ...
  while(1)
  {
  	...
  	if (kbhit())
  	{
  	 switch(getch())
	   {
	   	case 'b': pos > 0 &&ram[107]==0&&ram[7*16+3+9+128]==0? pos-- : pos;break;
	   	case 'm': pos<6 &&ram[107]==0&&ram[7*16+3+9+128]==0? pos++ : pos; break;
